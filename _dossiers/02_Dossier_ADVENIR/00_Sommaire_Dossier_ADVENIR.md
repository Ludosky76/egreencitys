# DOSSIER ADVENIR — EGREENCITY'S
## Programme ADVENIR — Voirie publique Guyane — 20 PDC — 10 × e-Premium AC 2×22 kW

---

## Synthèse du dossier

| | |
|---|---|
| **Bénéficiaire** | EGREENCITY'S — SAS |
| **SIREN** | 878 682 854 |
| **Siège** | 1 rue Akangoue — Résidence La Rougerie — 97355 Macouria-Tonate |
| **Dispositif** | ADVENIR — « Voirie publique » |
| **Région** | Guyane française (DROM — RUP) |
| **Nombre de stations** | 10 (10 × e-Premium AC 2×22 kW) |
| **Nombre de PDC** | **20** |
| **Prime unitaire** | 1 860 €/PDC |
| **Prime totale demandée** | **37 200 €** |
| **Investissement total (hors taxes Guyane)** | 58 020 € HT |
| **Taux de couverture ADVENIR** | **64,1 % du HT** |

---

## Pièces du dossier

### A — Pièces administratives

| # | Pièce | Source | Statut |
|---|-------|--------|--------|
| A1 | K-bis EGREENCITY'S (< 3 mois) | Greffe Tribunal Cayenne | ✅ disponible (15/11/2019) |
| A2 | Statuts de la société | Archives société | ☐ |
| A3 | RIB compte professionnel | Banque entreprise | ☐ |
| A4 | CNI du Président (recto/verso) | Loic LUDOSKY | ☐ |
| A5 | Attestation URSSAF de vigilance | urssaf.fr | ☐ |
| A6 | Attestation fiscale DGFiP | impots.gouv.fr | ☐ |
| A7 | Qualification IRVE installateur | Qualifelec / Qualiopi | ☐ |

### B — Pièces techniques (à demander à E-TOTEM — voir email J1)

| # | Pièce | Fournisseur | Statut |
|---|-------|-------------|--------|
| B1 | Fiche technique e-Premium AC | E-TOTEM | ☐ |
| B2 | Fiche technique e-Fast S S120-2 | E-TOTEM | ☐ |
| B3 | Attestation OCPP 1.6J (exigence ADVENIR) | E-TOTEM | ☐ |
| B4 | Attestations MID (comptage énergie) | E-TOTEM | ☐ |
| B5 | Certification NF / IEC 61851 | E-TOTEM | ☐ |
| B6 | Attestation fournisseur agréé ADVENIR | E-TOTEM | ☐ |

### C — Pièces projet (préparées dans ce dossier)

| # | Fichier | Contenu |
|---|---------|---------|
| C1 | `01_Fiche_Projet_EGREENCITYS.md` | Présentation synthèse du projet | ✅ |
| C2 | `02_Tableau_20_PDC.xlsx` | Détail des 20 PDC (station, commune, type, puissance, coût) | ✅ |
| C3 | `03_Plan_Implantation_Guyane.md` | Répartition géographique + justification | ✅ |
| C4 | `04_Memoire_Technique.md` | Architecture technique et supervision OCPP | ✅ |
| C5 | `05_Engagements_Operateur.md` | Engagement signalétique + disponibilité 5 ans + ouverture au public | ✅ |
| C6 | `06_Modele_Lettre_Intention_Mairie.txt` | Modèle à adresser à chaque commune partenaire | ✅ |
| C7 | `07_Budget_Detaille.md` | Décomposition du budget par station | ✅ |
| C8 | `08_Lettre_Demande_ADVENIR.md` | Lettre formelle de demande à AVERE-France (à signer) | ✅ |

### D — Pièces partenaires (à collecter)

| # | Pièce | Source | Statut |
|---|-------|--------|--------|
| D1 | Lettres d'intention signées des maires | 9 communes cibles | ☐ |
| D2 | Conventions d'occupation domaine public | Communes | ☐ |
| D3 | Arrêtés municipaux autorisant les stations | Communes | ☐ |
| D4 | Plans de voirie des sites d'implantation | Services techniques mairies | ☐ |
| D5 | Attestations de raccordement EDF (pré-étude) | EDF SEI Guyane | ☐ |

---

## Calendrier du dépôt

| Étape | Action | Délai |
|-------|--------|-------|
| J+0 | Création compte ADVENIR | Aujourd'hui |
| J+1 à J+7 | Collecte pièces administratives (A1-A7) | 1 semaine |
| J+3 à J+14 | Réception pièces techniques E-TOTEM (B1-B6) | 2 semaines |
| J+7 à J+21 | Envoi lettres d'intention aux mairies (D1) | 3 semaines |
| J+21 à J+45 | Réception retours mairies + conventions (D2-D4) | 6 semaines |
| J+45 | **Dépôt du dossier complet** ADVENIR | 6-7 semaines après J1 |
| J+45 à J+90 | Instruction AVERE-France | 4-8 semaines |
| J+90 | **Notification d'éligibilité** | ~3 mois après J1 |
| J+90+ | Commande E-TOTEM (après notification) | — |

---

## Contacts ADVENIR

- **Plateforme** : https://advenir.mobi
- **Support** : contact@advenir.mobi
- **AVERE-France** : 01 53 38 42 00
- **Référent Outre-mer** : à identifier via formulaire contact

## Argument fort Guyane

> La Guyane est une **Région Ultrapériphérique (RUP)** de l'Union Européenne.
> À ce titre, les projets d'infrastructure énergétique y bénéficient d'une
> éligibilité renforcée à ADVENIR et aux fonds FEDER (taux majoré 85 %).
> Le présent projet participe de la transition énergétique du territoire,
> avec un maillage couvrant les 3 principales zones urbaines (île de
> Cayenne, Kourou, Saint-Laurent-du-Maroni) + une station DC rapide
> sur l'axe RN1 reliant les 3 pôles.
